...
### 8.5 DBM-09/10 실측 결과 (2026-05-19) ✅

**실행 환경**: 운영 VM `hajime` → Cloud SQL `postgre-sql` (qraku DB).

**도구 변경 — pgloader 포기 → 커스텀 Python migrator**:
- pgloader 3.6.7 (Ubuntu apt) 의 QMYND 라이브러리가 MySQL 8 default auth plugin (caching_sha2_password) 의 초기 핸드셰이크에서 실패 (`MYSQL-UNSUPPORTED-AUTHENTICATION`). mysql_native_password 사용자로 교체해도 동일 — 서버의 default plugin 협상 단계에서 abort.
- 대체: `tools/pg_data_migrator.py` 신규 (PyMySQL + psycopg2 + SQLAlchemy 메타데이터 reflect 기반, 약 180 LOC). PG schema 는 DBM-08 의 `init_pg_schema.py` 로 이미 만들어져 있으므로 데이터만 옮김.
- ADR-007 (pgloader 선택) 은 superseded — 후속 ADR 또는 본 §8.5 가 정본.

**DBM-09 데이터 이전 결과**:
- 28 테이블 / 466 행 / **3.01초**
- TRUNCATE (FK 역순) → INSERT (FK 정순) → 시퀀스 재설정 → ANALYZE
- 모든 테이블 row count 동일 (post-check 에서 확인)

**미세 이슈 + 조치**:
- MySQL JSON 컬럼이 PyMySQL 로 자동 dict/list 파싱 → PG TEXT 컬럼에 INSERT 시 `can't adapt type 'dict'` 에러. → migrator 에서 dict/list 값은 `json.dumps(...)` 후 INSERT 하도록 수정.

**DBM-10 검증 결과** (`tools/migration_check.py`):

| # | 항목 | 결과 |
|---|---|---|
| 1 | 행 수 일치 (28 테이블) | ✅ PASS |
| 2 | MAX(id) 일치 | ✅ PASS |
| 3 | PG sequence ≥ MAX(id)+1 | ✅ PASS |
| 4 | FK orphan 0 (MySQL/PG 각각) | ✅ PASS |
| 5 | 인코딩 sample (한/일/중) 동일 | ✅ PASS |
| 6 | JSON 컬럼 파싱 동일성 | ✅ PASS |
| 7 | 인덱스 / UNIQUE 컬럼셋 일치 | ✅ PASS (인덱스 보강 후, 아래 참고) |

**인덱스 보강 — `migration_sqls` 추가 10건** (`backend/database.py:219~228`):

| 테이블 | 누락된 인덱스 (MySQL 자동 생성, PG 누락) |
|---|---|
| globalreview | store_id, customer_id, order_id |
| menu | store_id |
| orderitem | order_id |
| pointhistory | related_order_id, store_id, customer_id |
| tabehoudaisession | group_id |
| table | store_id |

근본 원인: MySQL 은 FK 컬럼에 자동으로 보조 인덱스를 만들지만, SQLModel 은 `Field(index=True)` 가 명시된 경우만 인덱스 생성. PG schema (DBM-08 의 init_db) 에 누락 → 본 보강으로 양 DB 동일성 회복.

**예상되는 PG-only 인덱스 (경고 무시 OK)**:
- `store.line_id`, `store.google_id` (외부 ID UNIQUE — DBM-05 보강)
- `order.idempotency_key` (INF-03 멱등성)
- `order.guest_uuid` (게스트 식별)
- `orderitem.tabehoudai_session_id` (食べ放題)

**산출물**:
- `tools/pg_data_migrator.py` 신규 (DBM-12 컷오버에서도 재사용)
- `backend/database.py` 의 `migration_sqls` 에 FK 인덱스 10건 추가
- Cloud SQL `qraku` DB 에 실제 운영 데이터 반영됨 (이번은 리허설, 운영 컷오버는 DBM-12 에서 다시)

**ADR-007 status update 권장**: superseded by pg_data_migrator.py (작은 데이터셋 + MySQL 8 호환성 이유).
