...
| 날짜 | 스테이지 | 결과 |
|---|---|---|
| 2026-06-09 | store_id 통일 2차 코드리뷰(GPT-5.5) 반영 | 11개 지적 전부 반영. **Crit**: ①기존주문 backfill(slug→숫자ID)+고아정리, ②admin 환불 교차매장 우회 제거(store_id 직접), ③pay_order 잔여조회 store.id(조기 close 방지). **Imp**: ④NOT NULL+실제 FK 강제, ⑤죽은 멱등쿼리 제거, ⑥insights neighbor NameError(500) 수정, ⑦register 잔여 resolve 2곳, ⑧super_admin 리포팅 store_id 전환, ⑨pickup_code store_id(네임스페이스 분리 해소). **Min**: ⑩NULL 폴백 제거, ⑪demo cleanup(backfill 로 해소). 정적검사 pyflakes 도입→신규 undefined 0. 검증: import OK·pytest 39·build OK·신규 lint 0. 부수 발견(기존버그·후속분리): register `today` NameError·beta.py `now_utc_naive`·orders `.cast(int)`·loyalty `final_price`. 커밋 7e37062·5481e6d·5e60f93 |
| 2026-06-09 | Order↔Store store_id 통일 (리뷰 Imp2 근본해결) | 출시 전·데이터 없음 → FK 통합 즉시 진행. `Order.store_id:int FK` 신설(+마이그레이션), 생성 시 store_id 세팅 + shop_id dual-write. 주문↔매장 매칭(candidate-set slug/str(id) 이중분기)을 정규 store_id 로 통일: orders(결제·알림·정산)·stats(9)·insights·register(정산)·tables·demo·admin·discover·takeout_wait. polymorphic 헬퍼 `order_store.py` 삭제. super_admin 리포팅 group-by·pickup_code 네임스페이스는 dual-write 로 정상 동작하여 유지(버그 재발과 무관). 검증: AST OK·라우터 import OK·pytest 39/39. 커밋 8db5a60·2adb3fc. (shop_id 컬럼 물리 제거는 후속.) |
