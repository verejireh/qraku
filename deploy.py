...
SSH_KEY_PATH = os.path.expanduser("~/.ssh/qraku")  # SSH 키를 안전한 위치로 이동
...
def create_deploy_package():
    ...
    try:
        subprocess.run("npm install && npm run build", shell=True, check=True, cwd="frontend-react")
        print("✅ 로컬 빌드 성공!")
    ...
    with zipfile.ZipFile(ZIP_FILENAME, 'w', zipfile.ZIP_DEFLATED) as zipf:
        # 프로젝트 루트의 backend와 frontend-react 폴더 포함
        for target in ['backend', 'frontend-react', 'setup_server.sh']:
...
def deploy():
    ...
    try:
        ...
        try:
            ...
            subprocess.run(["uv", "pip", "install", "paramiko"], check=True)
        import paramiko
    ...
    try:
        ...
        remote_cmd = " && ".join([
            ...
            "python3 -m venv --help > /dev/null 2>&1 || (sudo apt-get install -y python3-venv -qq)",
