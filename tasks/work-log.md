...
## DBM-11 — Auth Proxy 영구 설치 준비 (자료 작성)

**완료**: 2026-05-19 (자료 작성만; 실 설치는 자이라가 별도 시점에 실행) / **owner**: postgres-specialist (claude-opus-4-7)

### 산출물

1. **`tools/cloud-sql-proxy.service`** — systemd unit 파일
   - Type=simple, Restart=always
   - GCE 메타데이터 SA 자격 사용 (기본). 대안 `--credentials-file` flag 주석으로 안내.
   - `--health-check --http-port=9090` 으로 liveness/readiness/startup 엔드포인트 노출
   - `--structured-logs` 로 journald 통합
   - 보안: NoNewPrivileges, ProtectSystem=strict, PrivateTmp
2. **`docs/deployment.md` §11.4 갱신**:
   - 인증 경로 A (VM scope 확장, 1-2분 다운타임) vs B (SA 키 파일, 다운타임 0) 비교
   - 경로 A 의 gcloud 명령 풀세트
   - 바이너리 + systemd 설치 절차
   - 검증 명령 (psql + http://127.0.0.1:9090/readiness)
   - 트러블슈팅 5가지 (PERMISSION_DENIED, scope 부재, 포트 충돌, asyncpg SSL, network wait)
   - 운영 명령 (journalctl, systemctl, health check)
3. **`tasks/current-tasks.md` DBM-11 카드 갱신**: 현황 + 두 인증 경로 + 공통 설치 절차 + 수용 기준 5가지.

### 실 설치는 자이라가 직접 (다운타임 동반)

- 경로 A 선택 시: VM stop/start 약 1-2분 다운타임. 매장 비영업 시간대 (예: 새벽 2-5시) 권장.
- 설치 후 `psql 'host=127.0.0.1 ...' -c "select 1"` 정상이면 backend `.env` 의 `DATABASE_URL` 을 `postgresql+asyncpg://ilhae:***@127.0.0.1:5432/qraku` 로 교체 가능 (외부 IP / SSL 옵션 불필요).
- DBM-12 (실 컷오버) 룬북 §3 의 "Auth Proxy 옵션" 이 동작하게 됨.

### 비고

- 본 카드의 systemd unit 은 `--address=127.0.0.1` 로 localhost only — 외부 노출 차단.
- 운영 VM 의 다른 서비스 (예: 로컬 MySQL 의 다른 클라이언트) 가 5432 안 쓰는지 한 번 확인 후 enable.
- 이번 사이클의 DBM-09/10 시 사용했던 임시 cloud-sql-proxy (홈 디렉토리에 두고 ad-hoc 실행) 는 본 systemd unit 적용 후 정리.


---

## DBM-11 + OPS-05 실행 완료 (2026-05-19)

**완료**: 2026-05-19 / **owner**: 자이라 + Claude (co-execution)

### DBM-11 Auth Proxy 영구 설치

#### Phase A — VM SA scope 확장

처음에 scope alias 오타 (`servicecontrol` 무효) 로 1회 실패 → 단순화: scope 를 `cloud-platform` 단일로 (모든 GCP API 포괄). 2번째 stop/set-scopes/start 로 성공.

```
VM stop → set-service-account --scopes=cloud-platform → start
SA: 351518233402-compute@developer.gserviceaccount.com
IAM: roles/cloudsql.client 부여 (기존 roles/editor 와 별개)
```

#### Phase B — systemd unit 설치

- `tools/cloud-sql-proxy.service` 운영 VM 으로 scp
- `cloud-sql-proxy` 바이너리 (v2.13.0) `/usr/local/bin/` 설치
- systemd enable + start → `active (running)`
- 5432 (PG) + 9090 (health) 정상 listen
- 첫 부팅 로그: `Authorizing with Application Default Credentials` → `Listening on 127.0.0.1:5432` → `Starting health check server at localhost:9090`

#### Phase C — 접속 검증

- PGPASSWORD env 로 시도 → password authentication failed (이유 불명, bash quoting 의심)
- `psql -W` (interactive password prompt) 로 시도 → ✅ ilhae + postgres 모두 성공
- 결론: proxy + 인증 모두 정상. backend 는 .env 파일에서 비번 읽으므로 영향 없음.

#### 보안 노트

자이라가 GCP 콘솔에서 새 ilhae 비번을 postgre-sql 의 기본 (postgres 슈퍼유저용) 비번과 동일하게 설정. 임시 운영엔 OK, 컷오버 후 별도 비번으로 분리 권장 (별도 카드).

### OPS-05 마무리

#### deploy.py 완료 후 발견된 문제 3가지

1. **REDIS_URL 환경변수 누락** — INF-01 (Redis) 코드 머지됐지만 `.env` 에 변수 미설정 → `CRITICAL ERROR: REDIS_URL 환경변수가 설정되지 않았습니다`. backend startup 실패.
   - 해결: `echo 'REDIS_URL=redis://localhost:6379/0' >> ~/qr-order-system/backend/.env`
2. **MySQL 에서 `IF NOT EXISTS` 구문 미지원 경고** — 약 7건 (DBM-05 의 ANSI 호환화는 PG 용; MySQL 은 ALTER ADD COLUMN / CREATE INDEX 에 IF NOT EXISTS 지원 안 함). 이건 ⚠️ Migration skipped 로 표시될 뿐 fatal 아님. 컷오버 후 PG 로 옮기면 모두 정상.
3. **PID 5922 가 8003 점유 + systemctl restart loop** — 옛 부팅이 살아남았고 새 시작이 bind 못 함. `systemctl stop` + `kill PID` + `systemctl start` 로 정리.

#### 검증

- `redis-cli ping` → PONG
- `systemctl status qrorder` → active (running), 단일 PID
- `curl /api/healthz` → 200
- `curl /api/readyz` → 200 (DB + Redis ping 통과)

### 결과 + 사이클 상태

DBM-08b (PG 통합 부팅) 진행 가능 상태. DBM-12 F-2 (실 컷오버) 도 룬북대로 실행 가능.
