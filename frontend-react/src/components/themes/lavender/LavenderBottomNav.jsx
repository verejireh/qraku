...
export default function LavenderBottomNav() {
    ...
    const navItems = [
        ...
        { icon: 'list_alt', label: t('orders') || 'Orders', path: getPath('orders'), active: location.pathname.includes('/orders') }, // receipt_long -> list_alt per stitch
        { icon: 'payments', label: t('checkout') || 'お会計', path: getPath('checkout'), active: location.pathname.includes('/checkout') },
    ]

    return (
        <footer
            className="fixed bottom-0 left-0 right-0 z-[60] border-t border-[#9c7aff]/10"
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex flex-col items-center gap-1 flex-1 transition-colors ${isActive ? 'text-[#9c7aff]' : 'text-[#6a5e8d] hover:text-[#9c7aff]'}`}
                        >
                            <span className="material-symbols-outlined" style={isActive && item.icon === 'restaurant_menu' ? { fontVariationSettings: "'FILL' 1" } : {}}>
                                {item.icon}
                            </span>
